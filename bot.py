...
# Загрузка .env переменных (если вдруг ты еще используешь .env)
load_dotenv()
...
try:
    ...
    raise ValueError("❌ Ошибка: Не найден раздел [BOT] или параметр TOKEN в config.ini")

if not bot_token:
    raise ValueError("❌ BOT TOKEN не найден в config.ini!")
